...
class BudgetSettingsTile extends AbstractSettingsTile {
  ...
  const BudgetSettingsTile.navigation({super.key, required this.icon, required this.title, required this.route, this.description, this.iconColor})
    : onPressed = null,
      value = null,
  ...
  Widget build(BuildContext context) {
    ...
    if (route != null) {
      ...
      trailingWidget = Text(value!, style: TextStyle(color: AppColors.secondaryTextColor, fontSize: 16));
    }

    VoidCallback? tapCallback;

    if (route != null) {
      tapCallback = () => Navigator.of(context).pushNamed(route!);
