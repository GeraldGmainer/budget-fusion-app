import 'package:flutter/material.dart';

class AppDimensions {
  static const double verticalPadding = 16.0;
  static const double horizontalPadding = 6.0;
  static const double floatingPadding = 10.0;

  static const double formWidth = 400.0;
  static const EdgeInsets formPadding = EdgeInsets.symmetric(horizontal: 40, vertical: 40);
  static const EdgeInsets pageCardPadding = EdgeInsets.symmetric(horizontal: horizontalPadding, vertical: verticalPadding);
  static const EdgeInsets pageFloatingPadding = EdgeInsets.symmetric(horizontal: horizontalPadding + floatingPadding, vertical: verticalPadding);

  static const double tabIconSize = 30;
  static const double iconSize = 14;
  static const double settingIconSize = 18;

  static const double tileIconWidth = 40;
  static const double tileIconSize = 24;
}
